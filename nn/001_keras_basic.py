...
with timer('training'):
    ...
    for i, (trn_idx, val_idx) in enumerate(cv.split(X_train, y_train)):
        ...
        X_trn = [X_trn[f].values for f in X_trn.columns]
        y_trn = y_train[trn_idx].values.reshape(-1, 1).astype(float)
        X_val = X_train.iloc[val_idx].copy()
        X_val = [X_val[f].values for f in X_val.columns]
        y_val = y_train[val_idx].values.reshape(-1, 1).astype(float)
        X_tst = X_test.copy()
        X_tst = [X_tst[f].values for f in X_tst.columns]
        ...
        with timer('fit'):
            ...
            model.fit(X_trn, y_trn, batch_size=256, epochs=100, validation_data=(X_val, y_val), callbacks=[roc])
        
        p = model.predict(X_val)
        val_series.iloc[val_idx] = p
        cv_results.append(roc_auc_score(y_val, p))
        test_df[i] = model.predict(X_tst)
